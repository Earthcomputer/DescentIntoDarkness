...
public record PickaxeItemCreator(
    Tier tier
) implements ItemCreator {
    public static final MapCodec<PickaxeItemCreator> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
        ToolTier.TIER_CODEC.fieldOf("tier").forGetter(creator -> creator.tier)
    ).apply(instance, PickaxeItemCreator::new));

    @Override
    public ItemCreatorType<?> type() {
        return ItemCreatorType.PICKAXE.get();
    }
